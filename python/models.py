...
class SRNN(nn.Module):
    ...
    def __init__(self, n_in, n_rec, n_out, n_t, thr, tau_m, tau_o, b_o, gamma, dt, model, classif, w_init_gain, lr_layer, t_crop, visualize, visualize_light, device):    
        ...
        self.hidden_lif=LIFNode(threshold=0.6, tau=2000.)
        #Parameters
        self.w_in  = nn.Parameter(torch.Tensor(n_rec, n_in ))
        self.OWM_in= OWMLayer([n_in,n_rec],1.0)
        self.w_rec = nn.Parameter(torch.Tensor(n_rec, n_rec))
        self.OWM_rec = OWMLayer([n_rec, n_rec], 1.0)
        self.w_out = nn.Parameter(torch.Tensor(n_out, n_rec))
        self.OWM_out = OWMLayer([n_rec, n_out], 1.0)
    ...
    def reset_parameters(self, gain):
        ...
        self.w_rec.data=self.w_rec.data*(1-torch.eye(self.n_rec)) # contain a question: the diag of wr is zero,but during traning, it is not zero anymore.
    ...
    def forward(self, x, yt, do_training):
        ...
        for t in range(self.n_t-1):     # Computing the network state and outputs for the whole sample duration
            ...
            a=torch.mm(self.z[t], self.w_rec.t()) + torch.mm(x[t], self.w_in.t())
            a=self.hidden_lif.tau *a
            spike = self.hidden_lif(a)
            self.v[t + 1] = self.hidden_lif.mem
            self.z[t + 1] = spike
        ...
        if do_training:
            self.grads_batch(x,self.z, yo, yt)
            
        return yo
    
    def grads_batch(self, x, z,yo, yt):
        ...
        #self.OWM_in.force_learn(self.w_in, torch.mean(z, dim=0), 2.0)
        self.OWM_rec.force_learn(self.w_rec,torch.mean(z,dim=0),2.0)
        #print(self.OWM_rec.P)
        #self.OWM_out.force_learn(self.w_out, torch.mean(z, dim=0), 2.0)
